...
const Login:React.FC<{onBack?:()=>void}> = ({onBack}) => {
  ...
  const submit=(e:React.FormEvent)=>{e.preventDefault();setError('');if(isRegister){if(!name||!email||!phone||password.length<6){setError('Preencha os campos. A senha precisa ter pelo menos 6 caracteres.');return;}if(!register(name,email,phone,password))setError('Não foi possível criar a conta.');}else if(!login(email,password))setError('E-mail ou senha incorretos.');};
  return <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div className="relative w-full max-w-md glass p-8 sm:p-10 rounded-[2rem] border border-white/10 shadow-2xl">
      ...
      {!isRegister&&<div className="mt-6 p-3 rounded-xl bg-slate-900 border border-white/5 text-[11px] text-slate-500 flex gap-2"><ShieldCheck size={15} className="text-emerald-500 shrink-0"/> Em uma versão comercial, conecte este login a um backend com autenticação segura.</div>}
    </div>
  </div>
};
